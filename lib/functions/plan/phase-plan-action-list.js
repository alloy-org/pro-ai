...
export class PhasePlanActionList {
   ...
   * @returns {Promise<Object>} - Plan result with actionItems and needsUserInput flag
   */
   ...
   * @returns {Promise<Object>} - Revised plan with actionItems
   */
...
Create concrete, actionable items based on the user's input. Include all necessary details.`;
  ...
  _formatContextForPrompt(gatheredContext) {
    ...
    if (gatheredContext.footnotes?.length > 0) {
      parts.push('\n**Rich Footnotes:**');
      for (const fn of gatheredContext.footnotes) {
        let fnDesc = `- "${fn.label}": ${fn.description || '(no description)'}`;
        if (fn.images?.length > 0) {
          fnDesc += ` [contains ${fn.images.length} image(s)]`;
        }
        if (fn.links?.length > 0) {
          fnDesc += ` [contains ${fn.links.length} link(s)]`;
        }
        parts.push(fnDesc);
      }
    }
